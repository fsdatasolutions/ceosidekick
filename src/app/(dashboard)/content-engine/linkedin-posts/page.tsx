// src/app/(dashboard)/content-engine/linkedin-posts/page.tsx
// Content Engine - LinkedIn Posts List
...
function truncateContent(content: string | null, maxLength: number = 120): string {
  if (!content) return "No content";
  if (content.length <= maxLength) return content;
  return content.substring(0, maxLength).trim() + "...";
}

export default async function LinkedInPostsPage({ searchParams }: PageProps) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-neutral-600">
                Create and manage short-form posts for engagement
              </p>
            </div>
          </div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="flex rounded-lg border border-neutral-200 overflow-hidden">
            ...
            <Link
              href="/content-engine/linkedin-posts?status=draft"
            ...
            <Link
              href="/content-engine/linkedin-posts?status=archived"
              className={`px-3 py-1.5 text-sm border-l border-neutral-200 flex items-center gap-1.5 ${
                status === "archived" 
                  ? "bg-neutral-100 text-neutral-900 font-medium" 
                  : "text-neutral-600 hover:bg-neutral-50"
      ...
      {posts.length > 0 ? (
        <>
          <div className="bg-white rounded-xl border border-neutral-200 divide-y divide-neutral-100">
            {posts.map((post) => (
              <Link
                key={post.id}
                href={`/content-engine/linkedin-posts/${post.id}`}
          ...
          {total > limit && (
            <div className="mt-6 flex items-center justify-center gap-2">
              {currentPage > 1 && (
                <Link href={`/content-engine/linkedin-posts?page=${currentPage - 1}${status ? `&status=${status}` : ""}`}>
        ...
        <div className="bg-white rounded-xl border border-neutral-200 p-12 text-center">
          ...
          <p className="text-sm text-neutral-600 mb-6 max-w-md mx-auto">
            {status 
              ? `You don't have any ${status} posts.`
              : "Create your first LinkedIn post to engage your professional network with short-form content."
            }
          </p>
          <div className="flex justify-center gap-3">
            ...
            <Link href="/content-engine/linkedin-posts/new?mode=manual">
